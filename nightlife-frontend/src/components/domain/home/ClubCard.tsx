...
import type { Club } from "@/lib/apiClient";
...
export function ClubCard({ club }: { club: Club }) {
  const hasImage = typeof club.profileImageUrl === "string" && club.profileImageUrl.trim().length > 0;
    ...
    >
      ...
      <div
        className="
          ...
          ring-2 ring-nl-secondary/60 ring-offset-2 ring-offset-[#000117]
          bg-black/20
        "
      >
        {hasImage ? (
          ...
          <Image
            ...
            onError={(e) => {
              ...
              if (el) {
                el.innerHTML = `<div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#6B3FA0]/20 to-black/60"><span class="text-white/80 font-semibold text-xl">${initials(club.name)}</span></div>`;
              }
            }}
          />
        )}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-start gap-2">
          ...
          <p className="text-white/80 text-sm leading-snug line-clamp-2">{club.address}</p>
        </div>
      </div>

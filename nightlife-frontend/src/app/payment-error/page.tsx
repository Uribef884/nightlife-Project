...
export default function PaymentErrorPage() {
  ...
  const fetchTransactionDetails = useCallback(async (transactionId: string) => {
    try {
      paymentDebugger.logLifecycleEvent('Starting transaction details fetch', 'PaymentError');
      paymentDebugger.logUrlParams(searchParams, 'PaymentError');
      ...
      paymentDebugger.logApiResponse(response, `${apiUrl}/checkout/unified/status/${transactionId}`, 'PaymentError');
      ...
      paymentDebugger.logApiResponse(data, `${apiUrl}/checkout/unified/status/${transactionId}`, 'PaymentError');
      ...
      paymentDebugger.logTransactionDetails(transactionDetails as PaymentDebugInfo, 'PaymentError');
      paymentDebugger.validateTransactionData(transactionDetails as PaymentDebugInfo, 'PaymentError');
      
      setTransactionDetails(transactionDetails);
    } catch (err) {
      paymentDebugger.logError(err, 'fetchTransactionDetails', 'PaymentError');
      setError('Error al cargar los detalles de la transacción');
    }
  }, [user?.email, searchParams]);

  useEffect(() => {
    paymentDebugger.logLifecycleEvent('Component mounted', 'PaymentError');
    ...
    if (storedDetails) {
      try {
        ...
        paymentDebugger.logStoredData('localStorage', 'lastTransactionDetails', details, 'PaymentError');
        
        // Log transaction details for debugging
        paymentDebugger.logTransactionDetails(details as PaymentDebugInfo, 'PaymentError');
        paymentDebugger.validateTransactionData(details as PaymentDebugInfo, 'PaymentError');
        ...
        paymentDebugger.logError(err, 'parseStoredDetails', 'PaymentError');
        setError('Error al cargar los detalles de la transacción');
      }
    } else if (transactionId) {
      // If we have a transaction ID but no stored details, try to fetch them
      paymentDebugger.logLifecycleEvent('Fetching transaction details from API', 'PaymentError');
      fetchTransactionDetails(transactionId);
    } else {
      // No transaction data available
      paymentDebugger.logLifecycleEvent('No transaction data available', 'PaymentError');
  ...
  const getErrorMessage = (errorCode?: string, errorMessage?: string) => {
    ...
    switch (errorCode) {
      case 'PAYMENT_ERROR':
        return 'Error interno del procesador de pagos';
      case 'NETWORK_ERROR':
        return 'Error de conexión con el procesador de pagos';
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-300 text-lg mb-4">
            Ocurrió un error técnico durante el procesamiento de tu pago.
          </p>

          <div className="bg-orange-900/20 border border-orange-500/50 rounded-lg p-4 max-w-md mx-auto mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                ...
                <span className="text-orange-400 font-medium text-sm sm:text-base">ERROR</span>
              </div>
              ...
              {transactionDetails.errorCode && (
                <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                  <span className="text-slate-300 text-sm sm:text-base">Código de error:</span>
                  <span className="text-slate-100 font-mono text-xs sm:text-sm">{transactionDetails.errorCode}</span>
                </div>
              )}
            </div>
        ...
        <div className="bg-slate-800/30 border border-slate-700/40 rounded-lg p-4 sm:p-6 text-center">
          <h3 className="text-slate-100 font-medium mb-2">¿Necesitas ayuda técnica?</h3>

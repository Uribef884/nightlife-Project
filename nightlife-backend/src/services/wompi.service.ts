...
import { generateCardTokenizationSignature, generateNequiTokenizationSignature, generateTransactionSignature } from '../utils/generateWompiSignature';
...
// export interface DaviplataTokenizationData { phone_number: string; } // DISABLED
...
export interface PSETransactionData {
  ...
  payment_method: {
    type: 'PSE';
    user_type: 0 | 1; // 0 = natural person, 1 = business
...
export interface BancolombiaTransferData {
  amount_in_cents: number;
  currency: string;
  reference: string;
  customer_email: string;
...
// export interface DaviplataTransactionData { // DISABLED
//   amount_in_cents: number;
//   currency: string;
//   reference: string;
//   customer_email: string;
...
class WompiService {
  ...
  // async tokenizeDaviplata(data: DaviplataTokenizationData): Promise<WompiTokenResponse> { // DISABLED
  //   // Generate integrity signature
  ...
  async checkTokenStatus(type: 'nequi' | 'bancolombia_transfer', tokenId: string): Promise<WompiTokenResponse> {
...
async createTransaction(payload: {
  ...
  async pollTokenStatus(
    type: 'nequi' | 'bancolombia_transfer',
    tokenId: string,
  ...
  async getPSEFinancialInstitutions(): Promise<PSEBanksResponse> {
    return this.makeRequest<PSEBanksResponse>('GET', WOMPI_CONFIG.ENDPOINTS.PSE.FINANCIAL_INSTITUTIONS);
  }

  async pollTransactionForAsyncUrl(
    transactionId: string,
    maxWaitTime = 30000 // 30 seconds for async URL
  ): Promise<string> {
    const start = Date.now();
    while (Date.now() - start < maxWaitTime) {
      const res = await this.getTransactionStatus(transactionId);
      const asyncUrl = res.data.payment_method?.extra?.async_payment_url;
      if (asyncUrl) return asyncUrl;
      
      await new Promise(r => setTimeout(r, 2000)); // Poll every 2 seconds
    }
    throw new Error(`Async payment URL not found for transaction ${transactionId}`);
  }
}

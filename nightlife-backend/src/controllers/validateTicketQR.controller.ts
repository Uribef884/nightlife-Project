...
export async function previewTicketQR(
  ...
  try {
    ...
    const user = req.user!;
    const validation = await validateTicketPurchase(qrCode, user, true); // isPreview = true
...
export async function confirmTicketQR(
  ...
  try {
    ...
    const user = req.user!;
    const validation = await validateTicketPurchase(qrCode, user, false); // isPreview = false (confirmation)
